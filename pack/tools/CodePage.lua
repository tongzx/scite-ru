--[[--------------------------------------------------
CodePage.lua
Authors: YuriNB, VladVRO, mozersЩ
Version: 2.3.2
------------------------------------------------------
√ибрид 2х скриптов:
win1251 to cp866 keyboard mapper (YuriNB icq#2614215)
 ѕереключатель кодировки текущего ввода и отображени€ win1251/dos866
 ќдновременно переключаетс€ кодировка окна консоли
и
codepage.lua (VladVRO)
 ѕоказ текущей кодировки в статусной строке.

 роме того, скрипт пытаетс€ отличить кодировку cp866 от win1251 при открытии файла или при переключении на него.
≈сли это удаетс€, то происходит автоматическое переключение отображени€ и ввода.
------------------------------------------------------
Connection:
...
function change_codepage_ru()
	...
	if props["character.set"]=='255' then
		...
		props["chars.accented"]='јаЅб¬в√гƒд≈е®Є∆ж«з»и…й кЋлћмЌнќоѕп–р—с“т”у‘ф’х÷ц„чЎшўщЏъџы№ьЁэёюя€'
	else
		props["character.set"]='255'
		props["chars.accented"]='А†Б°ВҐГ£Д§Е•рсЖ¶ЗІИ®Й©К™ЛЂМђН≠ОЃПѓРаСбТвУгФдХеЦжЧзШиЩйЪкЫлЬмЭнЮоЯп'
	end
...
-- ƒобавл€ем свой обработчик событи€ OnChar
