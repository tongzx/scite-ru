...
//!-start-[caret]
	...
	if(tmp_str.length()){
		SendEditor(SCI_SETCARETWIDTH, IntFromHexByte(tmp_str.c_str()));
		SendOutput(SCI_SETCARETWIDTH, IntFromHexByte(tmp_str.c_str()));
	}
	else{
//!-end-[caret]
...
//!-end-[BetterCalltips]
	...
	if (props.GetInt("margin.bookmark.by.single.click",1)==1) //!-add-[SetBookmark]
		SendEditor(SCI_SETMARGINSENSITIVEN, 1, 1);	//!-add-[SetBookmark]
