...
void SciTEBase::GrepRecursive(GrepFlags gf, FilePath baseDir, const char *searchString, const char *fileTypes, unsigned int basePath) { //!-change-[FindResultListStyle]
...
//!-start-[FindResultListStyle]
		if (props.GetInt("lexer.errorlist.findliststyle", 1)) {
			basePathLen = strlen(directory);
			os.append(directory);
			if (pathSepChar == os[os.length()-1]) {
				basePathLen--;
			} else {
				os.append(pathSepString);
			}
		}
//!-end-[FindResultListStyle]
		os.append(fileTypes);
