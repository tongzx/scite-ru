...
//!		if (!menuKey && ::IsDialogMessage(reinterpret_cast<HWND>(wParameters.GetID()), pmsg))
		if (!menuKey && DialogHandled(wParameters.GetID(), pmsg)) //!-change-[FixWideStringHandler]
			return true;
	}
...
BOOL SciTEWin::IncrementFindMessage(HWND hDlg, UINT message, WPARAM wParam) {
	...
	case WM_INITDIALOG:{
		...
		RegisterHotKey(hDlg,1,0,VK_F3);	//!-change-[IncrementalSearch.AddHotkeys]
		RegisterHotKey(hDlg,2,MOD_SHIFT,VK_F3);	//!-change-[IncrementalSearch.AddHotkeys]
...
//!-end-[IncrementalSearch.AddHotkeys]
	case WM_CLOSE:
		...
		UnregisterHotKey(hDlg,3);	//!-add-[IncrementalSearch.AddHotkeys]
		::SendMessage(hDlg, WM_COMMAND, IDCANCEL, 0);
		break;
