...
//!-end-[InputErr]
		...
		while (running) {
			...
			if ((bytesAvail < 1000) && (hWriteSubProcess != INVALID_HANDLE_VALUE) && (writingPosition < totalBytesToWrite)) {
				...
				/*size_t*/ int bytesToWrite = jobToRun.input.search("\n", writingPosition) + 1 - writingPosition; //-[Issue 34 fix]
...
//!-start-[position.autosave]
	cmdShow = props.GetInt("position.cmdShow", SW_SHOWNOACTIVATE);
	if (cmdShow != SW_MAXIMIZE) cmdShow = SW_SHOWNOACTIVATE;
//!-end-[position.autosave]
	if (width == -1 || height == -1) {
		cmdShow = SW_MAXIMIZE;
		width = CW_USEDEFAULT;
